...
def get_a_gs(freq_gs,kBT,time):
	...
	while counter<freq_gs.shape[0]:
		a_mat[counter,counter]=freq_gs[counter]/cmath.sin(freq_gs[counter]*tau_gs)
		counter=counter+1
	return a_mat
...
def get_a_ex(freq_ex,time):
	...
	while counter<freq_ex.shape[0]:
		a_mat[counter,counter]=freq_ex[counter]/cmath.sin(freq_ex[counter]*tau_ex)
		counter=counter+1
	return a_mat
...
def get_b_gs(freq_gs,kBT,time):
	...
	counter=0
	while counter<freq_gs.shape[0]:
		b_mat[counter,counter]=freq_gs[counter]/cmath.tan(freq_gs[counter]*tau_gs)
		counter=counter+1
...
def get_b_ex(freq_ex,time):
	...
	counter=0
	while counter<freq_ex.shape[0]:
		b_mat[counter,counter]=freq_ex[counter]/cmath.tan(freq_ex[counter]*tau_ex)
		counter=counter+1
...
def get_d_gs(freq_gs,kBT,time):
	...
	counter=0
	while counter<freq_gs.shape[0]:
		d_mat[counter,counter]=freq_gs[counter]*cmath.tanh(freq_gs[counter]*tau_gs/2.0)
		counter=counter+1
...
def get_d_ex(freq_ex,time):
	...
	counter=0
	while counter<freq_ex.shape[0]:
		d_mat[counter,counter]=freq_ex[counter]*cmath.tanh(freq_ex[counter]*tau_ex/2.0)
		counter=counter+1
...
def compute_full_response_func(freq_gs,freq_ex,Jmat,Kmat,E_adiabatic,kBT,steps,max_time,is_emission):
	...
	while counter<steps:
		...
		if is_emission:
				...
				chi_t=calc_chi_for_given_time(freq_ex,freq_gs,np.transpose(Jmat),eff_K,kBT,-current_t)
		else:
				# calculate the effective lineshape function as well as chi_t
				chi_t=calc_chi_for_given_time(freq_gs,freq_ex,Jmat,Kmat,kBT,current_t)
				g_inf=calc_lineshape_for_given_time(freq_gs,freq_ex,Jmat,Kmat,kBT,current_t)
	...
	while counter<steps:
		...
		response_func[counter,1]=chi[counter,1]*cmath.exp(1j*chi[counter,2]-1j*E_adiabatic*chi[counter,0])
		counter=counter+1
