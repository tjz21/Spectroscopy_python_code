...
class gbom:
    def __init__(self,freqs_gs,freqs_ex,J,K,E_adiabatic,dipole_mom,stdout):
        ...
        # emission equivalents of dipole moment and dipole derivative
        self.dipole_mom_emission=np.zeros(3)
        self.dipole_deriv_emission=np.zeros((freqs_gs.shape[0],3))
    ...
    # Finally, for HT effects, we also need to change the definiton of the dipole moment and transition 
    # dipole derivative so that they are effectively the ground state transition dipole moment and derivative
    def set_emission_variables(self):
		...
		# adjust dipole moment and dipole derivative
        self.dipole_deriv_emission[:,0]=np.dot(self.J_emission,self.dipole_deriv[:,0])
        self.dipole_deriv_emission[:,1]=np.dot(self.J_emission,self.dipole_deriv[:,1])
        self.dipole_deriv_emission[:,2]=np.dot(self.J_emission,self.dipole_deriv[:,2])
        self.dipole_mom_emission[0]=self.dipole_mom[0]+np.dot(self.K_emission,self.dipole_deriv[:,0])
        self.dipole_mom_emission[1]=self.dipole_mom[1]+np.dot(self.K_emission,self.dipole_deriv[:,1])
    ...
    def calc_fc_response(self,temp,num_points,max_time,is_emission,is_HT,stdout):
        kbT=const.kb_in_Ha*temp
        if is_emission:
             self.fc_response=franck_condon_response.compute_full_response_func(self.freqs_gs_emission,self.freqs_ex_emission,self.J_emission,self.K_emission,self.E_adiabatic,self.dipole_mom_emission,self.dipole_deriv_emission,kbT,num_points,max_time,is_emission,is_HT,stdout)
        else:
    ...
    def calc_eztfc_response(self,temp,num_points,max_time,is_qm,is_emission,is_HT,stdout):
        ...
        if is_emission:
            ztfc_resp=franck_condon_response.compute_full_response_func(self.freqs_gs_emission,self.freqs_ex_emission,self.J_emission,self.K_emission,self.E_adiabatic,low_temp_kbT,self.dipole_mom_emission,self.dipole_deriv_emission,num_points,max_time,is_emission,is_HT,stdout)
        else: 
